...
def reproduzir_gif(label, gif_path):
    frames = []
    i = 0
    try:
        while True:
    ...
    def animar(indice=0):
        label.configure(image=frames[indice])
        label.image = frames[indice]  # Evita garbage collection
        label.after(100, animar, (indice + 1) % len(frames))
...
def mostrar_pergunta():
    ...
    # Se for GIF, reproduzir animado
    if imagem_path.lower().endswith(".gif"):
        img_label = tk.Label(janela, bg=COR_FUNDO)
        img_label.pack(pady=5)
        threading.Thread(target=reproduzir_gif, args=(img_label, imagem_path), daemon=True).start()
    else:
