...
engine = pyttsx3.init()
engine.setProperty('rate', 300)  # Velocidade da fala
...
def reconhecer_fala():
    ...
    with sr.Microphone(device_index=1) as source: # se mic não funcionar, troque para sr.Microphone(device_index=1)
        falar("Estou a ouvir...")
        try:
